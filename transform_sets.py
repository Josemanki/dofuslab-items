    ...
    for bonus in bonuses[i-1]:
      if bonus['type']['name'] in NORMAL_STAT_MAP:
        transformed[i+1].append({
            'stat': NORMAL_STAT_MAP[bonus['type']['name']],
            'value': bonus['int_minimum'],
            'altStat': None
        })
  return transformed
...
def transform_sets():
  for set in en_dofusdude_data:
    if item_exists(set['name']):
      ...
      if 'effects' in set:
        ...
        en_set = find_localized_item(
            set['ankama_id'], en_dofusdude_data)
        fr_set = find_localized_item(
            set['ankama_id'], fr_dofusdude_data)
        es_set = find_localized_item(
            set['ankama_id'], es_dofusdude_data)
        de_set = find_localized_item(
            set['ankama_id'], de_dofusdude_data)
        it_set = find_localized_item(
            set['ankama_id'], it_dofusdude_data)
        pt_set = find_localized_item(
            set['ankama_id'], pt_dofusdude_data)
    ...
    with open('output/sets.json', 'w+', encoding='utf8') as outfile:
      outfile.write(json.dumps(dofuslab_sets_json +
                    final_sets, ensure_ascii=False))
      outfile.close()
